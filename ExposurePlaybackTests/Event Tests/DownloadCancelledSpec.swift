...
class DownloadCancelledSpec: QuickSpec {
    override func spec() {
        describe("DownloadCancelled") {
            let timeStamp: Int64 = 10
            let type = "Playback.DownloadCancelled"
            ...
            it("Should init and record complete structure") {
                let event = Playback.DownloadCancelled(timestamp: timeStamp, assetData: downloadId, downloadedSize: downloadedSize, mediaSize: mediaSize)
            ...
            it("Should produce correct jsonPayload") {
                let json = Playback.DownloadCancelled(timestamp: timeStamp, assetData: downloadId, downloadedSize: downloadedSize, mediaSize: mediaSize).jsonPayload
