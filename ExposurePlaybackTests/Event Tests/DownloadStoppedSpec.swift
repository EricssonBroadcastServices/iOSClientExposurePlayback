...
class DownloadStoppedSpec: QuickSpec {
    override func spec() {
        describe("DownloadStopped") {
            let timeStamp: Int64 = 10
            let type = "Playback.DownloadStopped"
            ...
            it("Should init and record complete structure") {
                let event = Playback.DownloadStopped(timestamp: timeStamp, assetData: downloadId, downloadedSize: downloadedSize, mediaSize: mediaSize)
            ...
            it("Should produce correct jsonPayload") {
                let json = Playback.DownloadStopped(timestamp: timeStamp, assetData: downloadId, downloadedSize: downloadedSize, mediaSize: mediaSize).jsonPayload
