...
class BufferingStartedSpec: QuickSpec {
    override func spec() {
        describe("BufferingStarted") {
            let timeStamp: Int64 = 10
            let offset: Int64 = 10
            let type = "Playback.BufferingStarted"
            
            it("Should init and record complete structure") {
                let event = Playback.BufferingStarted(timestamp: timeStamp, offsetTime: offset)
            ...
            it("Should produce correct jsonPayload") {
                let json = Playback.BufferingStarted(timestamp: timeStamp, offsetTime: offset).jsonPayload
