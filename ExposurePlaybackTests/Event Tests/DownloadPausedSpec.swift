...
class DownloadPausedSpec: QuickSpec {
    override func spec() {
        describe("DownloadPaused") {
            let timeStamp: Int64 = 10
            let type = "Playback.DownloadPaused"
            ...
            it("Should init and record complete structure") {
                let event = Playback.DownloadPaused(timestamp: timeStamp, assetData: downloadId, downloadedSize: downloadedSize, mediaSize: mediaSize)
            ...
            it("Should produce correct jsonPayload") {
                let json = Playback.DownloadPaused(timestamp: timeStamp, assetData: downloadId, downloadedSize: downloadedSize, mediaSize: mediaSize).jsonPayload
