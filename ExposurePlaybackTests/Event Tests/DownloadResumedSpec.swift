...
class DownloadResumedSpec: QuickSpec {
    override func spec() {
        describe("DownloadResumed") {
            let timeStamp: Int64 = 10
            let type = "Playback.DownloadResumed"
            ...
            it("Should init and record complete structure") {
                let event = Playback.DownloadResumed(timestamp: timeStamp, assetData: downloadId, downloadedSize: downloadedSize, mediaSize: mediaSize)
            ...
            it("Should produce correct jsonPayload") {
                let json = Playback.DownloadResumed(timestamp: timeStamp, assetData: downloadId, downloadedSize: downloadedSize, mediaSize: mediaSize).jsonPayload
