...
class PausedSpec: QuickSpec {
    override func spec() {
        describe("Paused") {
            let timeStamp: Int64 = 10
            let offset: Int64 = 10
            let type = "Playback.Paused"
            
            it("Should init and record complete structure") {
                let event = Playback.Paused(timestamp: timeStamp, offsetTime: offset)
            ...
            it("Should produce correct jsonPayload") {
                let json = Playback.Paused(timestamp: timeStamp, offsetTime: offset).jsonPayload
