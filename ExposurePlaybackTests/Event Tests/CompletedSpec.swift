...
class CompletedSpec: QuickSpec {
    override func spec() {
        describe("Completed") {
            let timeStamp: Int64 = 10
            let offset: Int64 = 10
            let type = "Playback.Completed"
            
            it("Should init and record complete structure") {
                let event = Playback.Completed(timestamp: timeStamp, offsetTime: offset)
            ...
            it("Should produce correct jsonPayload") {
                let json = Playback.Completed(timestamp: timeStamp, offsetTime: offset).jsonPayload
