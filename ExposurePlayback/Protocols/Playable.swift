...
public protocol Playable {
    ...
    func prepareSource(environment: Environment, sessionToken: SessionToken, adsOptions:AdsOptions? , callback: @escaping (ExposureSource?, ExposureError?) -> Void)
    ...
    /// - parameter sessionToken: `SessionToken` validating the user
    /// - parameter callback: Closure called on request completion
    func prepareSourceWithResponse(environment: Environment, sessionToken: SessionToken, adsOptions:AdsOptions?, callback: @escaping (ExposureSource?, ExposureError?, HTTPURLResponse?) -> Void)
}
...
extension Playable {
    ...
    /// - parameter sessionToken: `SessionToken` validating the user
    /// - parameter callback: Closure called on request completion
    public func prepareSourceWithResponse(environment: Environment, sessionToken: SessionToken, adsOptions:AdsOptions? = nil , callback: @escaping (ExposureSource?, ExposureError?, HTTPURLResponse?) -> Void) {
        prepareSource(environment: environment, sessionToken: sessionToken, adsOptions: adsOptions) { source, error in
